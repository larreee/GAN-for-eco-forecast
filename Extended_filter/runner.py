    ...
    for epoch in range(load_epoch+1, epochs+load_epoch+1):
        ...
        for di in range(d_iterations):
            for seq, encoded, labels in dataloader:
                ...
                with torch.no_grad(): 
                    generated_labels, agg_encoded =G(cat,(noise((bz,train_window,1),FT),
                                                                seq,
                                                                encoded))
                    generated_labels = pad(generated_labels,
                                           train_window,
                                           horizon)

                GP = gradient_penalty(D, generated_labels, labels, agg_encoded, seq, Lambda = 10, device = device, cat = cat)
                d_error = train_discriminator( D,
                                               d_optimizer,
                                               cat(labels, seq, agg_encoded).to(device),
                                               cat(generated_labels, seq, agg_encoded).to(device),GP)
                del labels, generated_labels, encoded, agg_encoded, GP
                i += 1
        ...
        for seq, encoded, labels in dataloader:
            ...
            g_error = train_generator( D,
                                       g_optimizer,
                                       cat(generated_labels, seq, agg_encoded).to(device))

            del generated_labels, encoded, agg_encoded, seq
            j += 1
        ...
        if epoch%1000 == 0: 
            save_checkpoint(G,D,
                            g_optimizer, d_optimizer,
                            epoch,
                            'tw-{}_hz-{}_bs-{}_di-{}_t-{}_B-{}_id-{}_f-{}'.format(train_window,
                                                                           horizon,
                                                                           batch_size,
                                                                           d_iterations,
                                                                           time_series,
